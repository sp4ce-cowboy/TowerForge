...
typealias StatisticUpdateActor = ((TFEvent) -> Void)?
...
struct StatisticUpdateLinkDatabase {
    ...
    func getStatisticUpdateActor<T: TFEvent>(for eventType: T.Type) -> StatisticUpdateActor? {
        let wrappedEvent = TFEventTypeWrapper(type: eventType)
        return statisticUpdateLinks[wrappedEvent]
    }
